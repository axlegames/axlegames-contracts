...
contract AxleToken3 is Context, IBEP20, Ownable {
    ...
    uint256 public maxtranscation = 50000000000000;
    ...
    address public feeWallet     = 0x46D3099487DDcf724df70ed11b40109dAb586F79;
    address public pinkAntiBot_  = 0xbb06F5C7689eA93d9DeACCf4aF8546C4Fe0Bf1E5;
    address public firstVerifier = 0x46D3099487DDcf724df70ed11b40109dAb586F79;
    ...
    uint256 public antiDumpAmount = _totalSupply.mul(5).div(10000);
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "AxleGames: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "AxleGames: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "AxleGames: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "AxleGames: Automated market maker pair is already set to that value");
    ...
    function _transfer(
        ...
        require( _isExcludedFromFees[sender] || _isExcludedFromFees[recipient]  || amount <= maxtranscation,"Max transaction Limit Exceeds!");

        if(antiBotEnabled){
            pinkAntiBot.onPreTransferCheck(sender,recipient,amount);
        }

        if(!_isExcludedFromFees[sender]) { require(tradingEnabled == true, "Trading not enabled yet"); }

        if(sender == owner()) { require(verifierOne == true && verifierOwner == true ,"not active"); }
         ...
         if (
            ...
            require(
                automatedMarketMakerPairs[sender] ||
                    _balances[recipient].add(amount) <= _totalSupply.div(10000),
                "AntiBot: Buy Banned!"
            );
        ...
        if(takeFee) {
            ...
            __transfer(sender, feeWallet, fees);
        }
